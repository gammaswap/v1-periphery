...
import { inTransaction } from "./gammapool-helper/expectEvent";
...
export function shouldBehaveLikeERC721(errorPrefix: any) {
  ...
  context('with minted tokens', function () {
    ...
    describe('balanceOf', function () {
      ...
      context('when querying the zero address', function () {
        it('throws', async function () {
          await expect(this.token.balanceOf(ethers.constants.AddressZero)).to.be.revertedWith("ERC721: INVALID OWNER");
        });
      });
    });
    ...
    describe('ownerOf', function () {
      ...
      context('when the given token ID was not tracked by this token', function () {
        ...
        it('reverts', async function () {
          await expect(this.token.ownerOf(tokenId)).to.be.revertedWith('ERC721: INVALID TOKENID');
        });
      });
    });
    ...
    describe('transfers', function () {
      ...
      const shouldTransferTokensByUsers = function (transferFunction: any) {
        ...
        context('when the address of the previous owner is incorrect', function () {
          it('reverts', async function () {
            await expect(transferFunction.call(this, this.other.address, this.other.address, tokenId, { from: this.owner.address })).to.be.revertedWith("ERC721: INCORRECT OWNER");
          });
        });

        context('when the sender is not authorized for the token id', function () {
          it('reverts', async function () {
            await expect(transferFunction.call(this, this.owner.address, this.other.address, tokenId, { from: this.other.address })).to.be.revertedWith("ERC721: FORBIDDEN");
        ...
        context('when the given token ID does not exist', function () {
          it('reverts', async function () {
            await expect(transferFunction.call(this, this.owner.address, this.other.address, nonExistentTokenId, { from: this.owner.address })).to.be.revertedWith("ERC721: INVALID TOKENID");
          });
        });

        context('when the address to transfer the token to is the zero address', function () {
          it('reverts', async function () {
            await expect(transferFunction.call(this, this.owner.address, ethers.constants.AddressZero, tokenId, { from: this.owner.address })).to.be.revertedWith("ERC721: FORBIDDEN");
          });
        });
      };
      ...
      describe('via safeTransferFrom', function () {
        ...
        const shouldTransferSafely = function (transferFun: any, data: any) {
          ...
          describe('to a valid receiver contract', function () {
            ...
            it('calls onERC721Received', async function () {
              ...
              await inTransaction(receipt.transactionHash, this.receiver, 'Received', {
              ...
              it('reverts', async function () {
                await expect(
                  transferFun.call(
                    ...
                    nonExistentTokenId,
                    { from: this.owner.address },
                  )).to.be.revertedWith('ERC721: INVALID TOKENID',
                );
              });
            });
          });
        ...
        describe('to a receiver contract returning unexpected value', function () {
          it('reverts', async function () {
            ...
            await expect(
              this.token.functions['safeTransferFrom(address,address,uint256)'](this.owner.address, invalidReceiver.address, tokenId, { from: this.owner.address })).to.be.revertedWith('ERC721: FORBIDDEN',
            );
          });
        });
        ...
        describe('to a receiver contract that reverts with message', function () {
          it('reverts', async function () {
            ...
            await expect(
              this.token.functions['safeTransferFrom(address,address,uint256)'](this.owner.address, revertingReceiver.address, tokenId, { from: this.owner.address })).to.be.revertedWith('ERC721ReceiverMock: reverting',
            );
          });
        });
        ...
        describe('to a receiver contract that reverts without message', function () {
          it('reverts', async function () {
            ...
            await expect(
              this.token.functions['safeTransferFrom(address,address,uint256)'](this.owner.address, revertingReceiver.address, tokenId, { from: this.owner.address })).to.be.revertedWith('ERC721: FORBIDDEN',
            );
          });
        });
        ...
        describe('to a contract that does not implement the required function', function () {
          it('reverts', async function () {
            ...
            await expect(
              this.token.functions['safeTransferFrom(address,address,uint256)'](this.owner.address, nonReceiver.address, tokenId, { from: this.owner.address })).to.be.revertedWith('ERC721: FORBIDDEN',
            );
          });
        });
      });
      ...
      describe('via safeMint', function () { // regular minting is tested in ERC721Mintable.test.js and others
        ...
        context('to a receiver contract returning unexpected value', function () {
          it('reverts', async function () {
            const invalidReceiver = await ERC721ReceiverMock.deploy('0x42000000', Error.None);
            await expect(
              this.token.functions['safeMint(address,uint256)'](invalidReceiver.address, tokenId)).to.be.revertedWith('ERC721: FORBIDDEN',
            );
          });
        });
        ...
        context('to a receiver contract that reverts with message', function () {
          it('reverts', async function () {
            const revertingReceiver = await ERC721ReceiverMock.deploy(RECEIVER_MAGIC_VALUE, Error.RevertWithMessage);
            await expect(
              this.token.functions['safeMint(address,uint256)'](revertingReceiver.address, tokenId)).to.be.revertedWith('ERC721ReceiverMock: reverting',
            );
          });
        });
        ...
        context('to a receiver contract that reverts without message', function () {
          it('reverts', async function () {
            const revertingReceiver = await ERC721ReceiverMock.deploy(RECEIVER_MAGIC_VALUE, Error.RevertWithoutMessage);
            await expect(
              this.token.functions['safeMint(address,uint256)'](revertingReceiver.address, tokenId)).to.be.revertedWith('ERC721: FORBIDDEN',
            );
          });
        });
        ...
        context('to a contract that does not implement the required function', function () {
          it('reverts', async function () {
            const nonReceiver = this.token;
            await expect(
              this.token.functions['safeMint(address,uint256)'](nonReceiver.address, tokenId)).to.be.revertedWith('ERC721: FORBIDDEN',
            );
          });
        });
      });
    ...
    describe('setApprovalForAll', function () {
      ...
      context('when the operator is the owner', function () {
        it('reverts', async function () {
          await expect(this.token.setApprovalForAll(this.owner.address, true, { from: this.owner.address })).to.be.revertedWith('ERC721: FORBIDDEN');
        });
      });
    });
    ...
    describe('getApproved', async function () {
      context('when token is not minted', async function () {
        it('reverts', async function () {
          await expect(
            this.token.getApproved(nonExistentTokenId)).to.be.revertedWith('ERC721: INVALID TOKENID',
          );
        });
      });
  ...
  describe('_mint(address, uint256)', function () {
    it('reverts with a null destination address', async function () {
      await expect(
        this.token.mint(ethers.constants.AddressZero, firstTokenId)).to.be.revertedWith('ERC721: FORBIDDEN',
      );
    });
    ...
    context('with minted token', async function () {
      ...
      it('reverts when adding a token id that already exists', async function () {
        await expect(this.token.mint(this.owner.address, firstTokenId)).to.be.revertedWith('ERC721: TOKEN EXISTS');
      });
    });
  });
  ...
  describe('_burn', function () {
    it('reverts when burning a non-existent token id', async function () {
      await expect(
        this.token.burn(nonExistentTokenId)).to.be.revertedWith('ERC721: INVALID TOKENID',
      );
    });
    ...
    context('with minted tokens', function () {
      ...
      context('with burnt token', function () {
        ...
        it('deletes the token', async function () {
          expect((await this.token.balanceOf(this.owner.address)).toNumber()).to.equal(2);
          await expect(
            this.token.ownerOf(firstTokenId)).to.be.revertedWith('ERC721: INVALID TOKENID',
          );
        });

        it('reverts when burning a token id that has been deleted', async function () {
          await expect(
            this.token.burn(firstTokenId)).to.be.revertedWith('ERC721: INVALID TOKENID',
          );
        });
      });
    });
  ...
  describe('metadata', function () {
    ...
    describe('token URI', function () {
      ...
      it('reverts when queried for non existent token id', async function () {
        await expect(
          this.token.tokenURI(nonExistentTokenId)).to.be.revertedWith('ERC721: INVALID TOKENID')
      });
